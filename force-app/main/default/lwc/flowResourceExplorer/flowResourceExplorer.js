...
const isReference = value => !value ? false : value.indexOf('{!') === 0 && value.lastIndexOf('}') === (value.length - 1);
const getDataType = currentText => isReference(currentText) ? flowComboboxDefaults.referenceDataType : flowComboboxDefaults.stringDataType;
const formattedValue = (value, dataType) => isReference(value) ? value : (dataType === flowComboboxDefaults.referenceDataType ? '{!' + value + '}' : value);
const removeFormatting = value => !value ? value : (isReference(value) ? value.substring(0, value.lastIndexOf('}')).replace('{!', '') : value);

export default class FlowResourceExplorer extends LightningElement {
    @api name;
    @api label;
    @api required = false;
    ...
    generateMergeFieldsFromBuilderContext(builderContext) {
        ...
        this.getTypes().forEach(curType => {
            ...
            if (typeParts.length && builderContext[typeParts[0]]) {
                ...
                console.error(curType + ' is undefined');
            }
        });
        let options = [];
    ...
    dispatchValueChangedEvent() {
        if(this._value && this.isDataSelected) {
    ...
    handleWindowClick(event) {
        if (event.path && !event.path.includes(this.template.host) || !this.selfEvent) {
